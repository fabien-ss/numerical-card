...
public class Model {
    ...
    @Expose
    private transient String prefixe = "";
    @Expose
    private transient int longPK = 7;
    @Expose
    ...
    public List getComposant(ResultSet resultSet) throws Exception {
        ...
        while (resultSet.next()) {
            ...
            Object ob = constructor.newInstance();
                ...
                for (Field field : fields) {
                    if(field.isAnnotationPresent(Correspondance.class)){
    ...
    String conditionAttributNonNulle(Field[] modelFields) throws Exception{
        ...
        for (int i = 0; i < modelFields.length; i++) {
            ...
            if(modelFields[i].getDeclaredAnnotation(Correspondance.class)==null)
                continue;
            Correspondance coresp=modelFields[i].getAnnotation(Correspondance.class);
            if(coresp.primarykey() | coresp.foreignkey()){
    ...
    public void executeUpdate(Connection c, String query) throws Exception {
        ...
        Statement statement = c.createStatement();
        int x = statement.executeUpdate(query);
        System.out.println(x + " line(s) modified.");
        try{
            c.commit();
        }
        catch(Exception e){
    ...
    public double doubleValue(Connection c, String query) throws Exception{
    ...
    public List executeQuery(Connection c, String query) throws SQLException, Exception{
        System.out.println("executeily_Query");
        System.out.println(this.url);
        boolean mine = true;
        if (c == null || c.isClosed()) 
        ...
        Statement statement = c.createStatement();
        ResultSet resultSet = statement.executeQuery(query);
        List vObjects = this.getComposant(resultSet);
        c.close();
